...
def get_dict_into_block_without_id(soup, string):
    block = soup.find(
        'h2', string=string
    ).find_parent('div', class_='data-table')
    if not block:
        return None
    data = {}
    if block:
...
def _get_or_create_obj(klass: models.Model, value):
    if value and ('No' not in value) and ('-' not in value):
        try:
            obj, _ = klass.objects.get_or_create(
                title=value
            )
            return obj
        except FieldError:
...
def _get_drive(value):
    if value:
        obj, _ = Drive.objects.get_or_create(
            title=value
        )
        return obj
    return
...
def _get_performance(performance_block: BeautifulSoup):
    ...
    acceleration_to_100 = _get_or_create_obj(
        AccelerationTo,
        _check_digit_and_split(data.get('Acceleration 0 - 100 km/h'))
    )
    drive = _get_drive(
        data.get('Drive')
    )
...
def _get_battery(block: BeautifulSoup):
    ...
    obj, _ = Battery.objects.get_or_create(
        ...
        battery_type=_get_or_create_obj(
...
def _get_data_car(html_page):
    ...
    battery = _get_battery(soup.find(
        'div',
        class_='data-table',
        id='battery'))
    performance = _get_performance(soup.find(
        'div',
        class_='data-table',
        id='performance'))
    soup_charging = soup.find(
        'div',
        class_='data-table',
        id='charging')
    ...
    dimensions_weight = _get_dimensions_weight(
        get_dict_into_block_without_id(
            soup,
