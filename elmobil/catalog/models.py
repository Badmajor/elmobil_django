...
class RangeEstimation(models.Model, VerboseNamePluralMixin, IterMixin):
    city_cold = models.IntegerField(
        verbose_name="Город в холодную погоду", help_text="км."
    )
    highway_cold = models.IntegerField(
        verbose_name="Трасса в холодную погоду", help_text="км."
    )
    combined_cold = models.IntegerField(
        verbose_name="Смешанный в холодную погоду", help_text="км."
    )
    city_mild = models.IntegerField(
        verbose_name="Город в теплую погоду", help_text="км."
    )
    highway_mild = models.IntegerField(
        verbose_name="Трасса в теплую погоду", help_text="км."
    )
    combined_mild = models.IntegerField(
        verbose_name="Смешанный в теплую погоду", help_text="км."
    )
...
class Charging(models.Model, VerboseNamePluralMixin, IterMixin):
    ...
    charge_time = models.DurationField(
        blank=True, null=True, verbose_name="Время зарядки (до 80%)", help_text="ч:м:с"
    )
...
class DimensionsWeight(models.Model, VerboseNamePluralMixin, IterMixin):
    length = models.IntegerField(verbose_name="Длина", null=True, help_text="мм")
    width = models.IntegerField(verbose_name="Ширина", null=True, help_text="мм")
    width_with_mirrors = models.IntegerField(
        verbose_name="Ширина с зеркалами", null=True, help_text="мм"
    )
    height = models.IntegerField(verbose_name="Высота", null=True, help_text="мм")
    wheelbase = models.IntegerField(
        verbose_name="Колесная база", null=True, help_text="мм"
    )
    weight_unladen = models.IntegerField(
        verbose_name="Вес без нагрузки", null=True, help_text="кг."
    )
    gross_weight = models.IntegerField(
        verbose_name="Полная масса", null=True, help_text="кг."
    )
    payload = models.IntegerField(
        verbose_name="Грузоподъемность", null=True, help_text="кг."
    )
    cargo_volume = models.IntegerField(
        verbose_name="Объем багажника", null=True, help_text="л."
    )
    cargo_volume_frunk = models.IntegerField(
        verbose_name="Объем переднего багажника", null=True, help_text="л."
    )
    tow_hitch = models.BooleanField(
        null=True, blank=True, verbose_name="Возможность установить фаркоп"
    )
...
class Miscellaneous(models.Model, VerboseNamePluralMixin, IterMixin):
    ...
    turning_circle = models.DecimalField(
        max_digits=3, decimal_places=1, verbose_name="Радиус поворота", null=True
    )
...
class Car(StrTitleMixin, models.Model):
    ...
    slug = models.CharField(max_length=256, verbose_name="Slug", unique=True)
    description = models.TextField(
        verbose_name="Описание",
        blank=True,
        null=True,
    ...
    year_release = models.IntegerField(verbose_name="Год начала выпуска", null=True)
